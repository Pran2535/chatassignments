...
const InputBox = ({ placeholder = "Reply to @Rohit Yadav", onSendMessage }) => {
  ...
  const toggleExtraIcons = () => {
    const newShowIconsState = !showIcons;
    setShowIcons(newShowIconsState);
    animateIcons(newShowIconsState ? 1 : 0);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      {/* New Document Share Icon */}
      <TouchableOpacity
        style={styles.iconButton}
        onPress={toggleExtraIcons}
        accessibilityLabel="Share document or media"
      >
        <MaterialIcons
          name="attachment"
          size={24}
          color={showIcons ? "#4CAF50" : "#666"}
        />
      </TouchableOpacity>
